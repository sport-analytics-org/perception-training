...
def batch_to_tensors(batch: dict[str, torch.Tensor], device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    return (images - IMAGE_MEAN.to(device)) / IMAGE_STD.to(device), masks
