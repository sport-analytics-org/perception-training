...
class MaskSample(TypedDict):
    image: Float[torch.Tensor, "3 height width"]  # noqa: F722
    mask: Float[torch.Tensor, "n_masks height width"]  # noqa: F722
...
def image_to_tensor(image: Image.Image) -> Float[torch.Tensor, "3 height width"]:  # noqa: F722
    array = np.asarray(image, dtype=np.float32) / 255.0
    image_tensor = torch.from_numpy(array).permute(2, 0, 1)
    return (image_tensor - IMAGE_MEAN) / IMAGE_STD


def bitfield_to_masks(bitfield: np.ndarray) -> Float[torch.Tensor, "n_masks height width"]:  # noqa: F722
