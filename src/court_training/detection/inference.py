...
def predict(
    ...
    image_sizes = torch.tensor([[height, width]] * len(variants), device=model.device)
    results = model.predict(images, image_sizes)
    if hflip:
        results[1]["boxes"][:, [0, 2]] = width - results[1]["boxes"][:, [2, 0]]
